...
from .exporters import get_exporter_class
from .forms import QueryForm
from .models import Query, QueryLog, MSG_FAILED_BLACKLIST
...
class SchemaView(PermissionRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if schema:
            return render_to_response('sqllab/schema.html',
                                      {'schema': schema_info(connection)})
        else:
            return render_to_response('sqllab/schema_building.html')
...
class CreateQueryView(PermissionRequiredMixin, ExplorerContextMixin, CreateView):
    ...
    template_name = 'sqllab/query.html'
...
class PlayQueryView(PermissionRequiredMixin, ExplorerContextMixin, View):
    ...
    def render(self):
        return self.render_template('sqllab/play.html', {'title': 'Playground', 'form': QueryForm()})
    ...
    def render_with_sql(self, request, query, run_query=True, error=None):
        ...
        return self.render_template('sqllab/%s.html' % template, query_viewmodel(request.user,
                                                                                   query,
                                                                                   title="Playground",
                                                                                   run_query=run_query,
                                                                                   error=error,
...
class QueryView(PermissionRequiredMixin, ExplorerContextMixin, View):
    ...
    def get(self, request, query_id):
        ...
        return self.render_template('sqllab/%s.html' % template, vm)
    ...
    def post(self, request, query_id):
        ...
        return self.render_template('sqllab/query.html', vm)
