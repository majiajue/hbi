...
def generate_report_action(description="从SQL查询生成CSV文件",):
...
def _build_zip(queries):
    ...
    zip_file = ZipFile(temp, 'w', ZIP_DEFLATED)
    for r in queries:
        zip_file.write('%s.csv' % r.title, CSVExporter(r).get_output() or "Error!")
    zip_file.close()
