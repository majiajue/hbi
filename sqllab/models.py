...
class Query(models.Model):
    title = models.CharField(max_length=255, verbose_name=u"标签")
    sql = models.TextField()
    description = models.TextField(null=True, blank=True, verbose_name=u"描述")
    created_by_user = models.ForeignKey(settings.AUTH_USER_MODEL, null=True, blank=True, on_delete=models.CASCADE, verbose_name=u"创建人")
    created_at = models.DateTimeField(auto_now_add=True, verbose_name=u"开始时间")
    last_run_date = models.DateTimeField(auto_now=True, verbose_name=u"结束时间")
    snapshot = models.BooleanField(default=False, help_text="在快照任务中包含(如果启用)", verbose_name=u"快照")
    connection = models.CharField(blank=True, null=True, max_length=128,
                                  help_text="用于此查询的数据库连接的名称(在设置中指定)。如果为空，将使用EXPLORER_DEFAULT_CONNECTION。", verbose_name=u"数据库")
